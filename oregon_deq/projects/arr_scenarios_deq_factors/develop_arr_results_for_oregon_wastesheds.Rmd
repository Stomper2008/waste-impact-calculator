...
```{r}
# the masses at end-of-life
masses_eol <- baseline_mass_profile

# creating a pseudo wasteshed that is Oregon total
temp_1 <-
  masses_eol %>%
  mutate(wasteshed = "Oregon total") %>%
  group_by(year, wasteshed, material, disposition, umbDisp) %>%
  summarise(
    tons=sum(tons),
...
wasteshed_sort_order <- c("Oregon total", unique(masses_eol$wasteshed))

saveRDS(
  wasteshed_sort_order,
  "intermediate_output/wasteshed_sort_order.RData"
)

# adding the Oregon total as a wasteshed to the end-of-life masses
masses_eol <-
  bind_rows(
    masses_eol,
    temp_1
  ) %>%
  mutate(wasteshed=factor(wasteshed, levels=wasteshed_sort_order)) %>%
  arrange(
    year, wasteshed, material, umbDisp, disposition
  )

rm(temp_1)

# those masses summed by wasteshed and umbrella disposition
...
ggplot()+
  ...
  theme_539()+
  geom_bar(
    data=masses_eol_by_umbDisp %>% filter(wasteshed=="Oregon total"),
    aes(
  ...
  geom_text(
    data=masses_eol_with_rr %>% filter(wasteshed=="Oregon total"),
    aes(
      x=material, 
      y=tons,
...
... note 2020/2/19... it doesn't!  dispose_all is low.  gotta work on that.
...
arr_summary_data_1 <-
  ...
  mutate(
    ...
    poss_impact_redcution_abs = dispose_all-optimal
  )
saveRDS(
  arr_summary_data_1,
...
# those overall pie charts
One trivial -- yet important -- thing is the pie chart of total impacts of materials compared to total impacts of waste.

Now two pie charts based on CBEI data.  First, one that just shows CBEI results.

```{r}
#Then figure out the total impacts by wasteshed (based on CBEI).
impacts_from_CBEI_by_metacategory <-
  data.frame(
    unitOM = "MTCO2E",
    year = 2016,
    meta_category = c("Electricity", "Fuels", "Services", "Materials"),
    impact = c(9.5, 19.6, 23.1, 36.5)*1000000,
...
#pie chart with materials portion exploded!
temp_1 <-
  mutate(
    impacts_from_CBEI_by_metacategory,
#    explodeFactor=-0.1
    explodeFactor = case_when(meta_category == "Materials" ~ 0.1, T~0)
  )
temp_2 <-
  temp_1[c(4,3,2,1),] %>%
  mutate(
    labelX = c(0.6, -0.1, -0.5, -0.29),
    labelY = c(0.2, -0.5, 0.1, 0.68),
    labelToPrint = paste(
      meta_category,
      "\n",
      format(impact/1000000, digits = 2),
      sep=""
    )
  )
ggplot()+
  ggtitle("Major sources of Oregon's\nconsumption-based emissions")+
  theme_539()+
  geom_arc_bar(
    data = temp_2,
    aes(
      ...
      r0 = 0, 
      r = 1,
      amount = impact/1000000, #impact in millions 
      fill = meta_category, 
#      color = meta_category,
      alpha = meta_category,
      explode = c(0,0,-0.1,0)
  ...
  coord_fixed()+
  # scale_color_manual(
  #   values=c("darkorange1", "yellowgreen",   "aquamarine4", "steelblue4")
  # )+
  scale_fill_manual(
    values=rev(myPal2)
  )+
  scale_alpha_manual(
    values=c(0.75,0.75,0.75,0.75)
  )+
  theme(
...
Now let's do the variant of that with the materials impacts split up into slivers.

```{r}
# the CBEI data
temp_3 <-
  select(temp_2, -unitOM, -year) 
# the non-material part of the CBEI data
temp_4 <-
  filter(temp_3, meta_category != "Materials")
# the material part of the CBEI data
temp_5 <-
  filter(temp_3, meta_category == "Materials") %>%
  rename(total_impact = impact)
# the non-material stuff will stay the same
# the material stuff needs to be divided up.
temp_6 <-
  full_join(
    data.frame(
      filter(
        select(total_impact_of_waste_by_scenario_with_savings, -unitOM),
        scenario == "RecoverMax" | scenario == "newSavings"
      ),
      meta_category = "Materials"
    ),
    temp_5,
    by = "meta_category"
  )
# figuring the portion of material impacts not in the waste stream
temp_7 <-
  summarise(
    group_by(
      temp_6,
      explodeFactor, labelX, labelY, labelToPrint
    ),
    impact = sum(impact),
    total_impact = mean(total_impact)
  ) %>%
  ungroup() %>%
  mutate(scenario = "materials outside of wastestream") %>%
  select(scenario, impact, total_impact, explodeFactor,
         labelX, labelY, labelToPrint
  )
# merging with total
temp_8 <-
  bind_rows(
...
# switching the order and fixing values
temp_9 <-
  temp_8[c(3,2,1),] %>% 
  mutate(
    impact = 
      ifelse(
        scenario == "materials outside of wastestream",
        total_impact - sum(temp_6$impact),
        impact
      )
  ) %>%
  select(-total_impact, -explodeFactor, -labelX, -labelY, -labelToPrint) %>%
  rename(meta_category = scenario)
# now merge that with 
temp_10 <-
  bind_rows(
    temp_9,
...
# ok, it's got the form I need to plot
```
This plot is pretty complex to construct.  Lots of manual data entry here.
...
temp_11 <-
  mutate(
    ...
    explodeFactor = 
      case_when(
        meta_category == "newSavings" ~ 0.2,
        meta_category == "RecoverMax" ~ 0.1,
        meta_category == "materials outside of wastestream" ~ 0.1,
        TRUE ~ 0
      ),
    labelX =
      case_when(
        meta_category == "newSavings" ~ 0.6,
        meta_category == "RecoverMax" ~ 0.515,
        meta_category == "materials outside of wastestream" ~ 0.65,
        TRUE ~ labelX
      ),
    labelY =
      case_when(
        meta_category == "newSavings" ~ 1.2,
        meta_category == "RecoverMax" ~ 0.62,
        meta_category == "materials outside of wastestream" ~ -0.2,
        TRUE ~ labelY
      ),
    labelToPrint =
      case_when(
        meta_category == "newSavings" ~
          paste(
            "Potential of additional\nrecovery: ",
            format(impact/1000000, digits = 2),
            sep=""
          ),
        meta_category == "RecoverMax" ~
          paste(
            "Unrecoverable\nimpacts of\nwaste: ",
            format(impact/1000000, digits = 2),
            sep=""
          ),
        meta_category == "materials outside of wastestream" ~
          paste(
            "Materials\noutside of\nwaste\nstream: \n",
            format(impact/1000000, digits = 2),
            sep=""
...
ggplot()+
  ggtitle("Potential of maximized\nrecovery to reduce\nstatewide emissions")+
...
#    size = 2.8
  ...
  scale_alpha_manual(
    values=c(0.75,0.75,0.25,0.5,0.25,0.75)
  )+
  scale_size_manual(
    values = 
      c(
